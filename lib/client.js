...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const P={esports:{thinking:['分析中…','选手进入状态','读图启动','规划中','开始运算'],'tool-read':['翻阅档案','信息来了','读到重点','资料到手','情报价值千金'],'tool-write':['写入成功！','漂亮下笔','修改落地','文件更新','果断一手'],'tool-search':['搜索展开','线索锁定','雷达全开','扫描中','定位目标'],'tool-terminal':['终端启动','执行中','进入战场','操作开始','快节奏'],'tool-delegate':['队友登场','双线作战','派兵中','团队协作','援军就位'],'tool-ask':['请求指示','暂停请示','选手提问','等拍板','请求确认'],'tool-other':['新操作','出其不意','注意这个'],success:['通过了！','一次成功！','漂亮！搞定','稳！'],error:['出错了…','翻车了','有失误','没成功'],approval:['等你批准','暂停等待','需要你决定'],done:['搞定！🎉','任务完成','稳稳收下','GG！'],slow:['思考有点久','这步慢了','卡住了？'],'streak-suc':['{n}连杀！','{n}连胜！','{n}连击达成'],'streak-fail':['{n}连败','连续{n}次失败'],'delegate-start':['援军登场！','队友就位'],'delegate-end':['队友归位','支援完成']},animal:{thinking:['停下来观察','竖起耳朵','嗅探中','评估地形'],'tool-read':['翻开落叶','嗅了嗅文件','仔细阅读','低头查看'],'tool-write':['开始筑巢','留下爪印','谨慎修改','搭建中'],'tool-search':['嗅探气息','展开搜索','仔细翻找','锁定目标'],'tool-terminal':['钻进洞穴','探索通道','操作机关','深入未知'],'tool-delegate':['呼唤同伴','一声长啸','猎手集结'],'tool-ask':['歪头看你','停下等待','乖巧等待'],'tool-other':['新动作','意外举动'],success:['捕获成功！','猎物到手','稳稳落地'],error:['挣脱了','陷阱没中','抖抖毛继续'],approval:['等待允许','蹲坐等指令'],done:['回到巢穴','心满意足','完美收官'],slow:['一动不动，像在冥想','陷入沉思'],'streak-suc':['{n}连击成功！','连续{n}次得手'],'streak-fail':['连续{n}次扑空'],'delegate-start':['同族赶来','援军到位'],'delegate-end':['同伴归队','重新集结']},deadpan:{thinking:['在想。不一定有用。','思考中。','它在想事'],'tool-read':['读文件。','又在翻代码','阅读中'],'tool-write':['写入了。','改动提交','文件更新'],'tool-search':['搜索中。','翻资料','扫描索引'],'tool-terminal':['跑命令了','终端启动','执行中'],'tool-delegate':['又叫人了','开子代理','呼叫支援'],'tool-ask':['停下来了','问你呢','等你批准'],'tool-other':['新动作','不太懂'],success:['通过了。','居然成功了','嗯，还行'],error:['出错了','报错挺清楚','下次来'],approval:['等批准。','它在等你'],done:['搞定了。','任务完成','收工'],slow:['超过12秒了','还在想'],'streak-suc':['{n}连成','连续{n}次成功'],'streak-fail':['{n}连败'],'delegate-start':['又叫人了','外包系列'],'delegate-end':['子代理回来了','希望没添乱']}};
		const PT={esports:'通过了！一次成功！',animal:'捕获成功！猎物到手',deadpan:'搞定了。'};
		...
		enq(ev){for(const e of ev)if(e&&typeof e.t==='string')this.queue.push(e);if(this.queue.length>32)this.queue.splice(0,this.queue.length-32);this.tick()},
