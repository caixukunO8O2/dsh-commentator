...
- **自定义人物卡**：预设「疾风 🎤 / 自然探员 🦉 / 冷淡 🖥️」，或自建人物
  ...
  `{表情} {名字}`，口头禅随机穿插
- **自定义宠物形象**：选择宠物种类（小猫/小狗/狐狸/机器人/团子）、身体颜色、
  点缀色、名字、装饰（派对帽/侦探帽）、眼镜、大小；解说时宠物会做出**对应动作**——
  成功跳起、失败耷拉沮丧、任务完成跳舞、思考冒「…」泡泡、搜索眼睛扫描、等待批准闭眼等。
  全部用内联 SVG + CSS 动画，不加载任何远程图片
- 字幕与宠物显示在页面**右下角**（悬浮层，`pointer-events: none`），宠物常驻、
  不遮挡输入框、审批按钮或主要内容；每条字幕显示约 2~4 秒后平滑消失
- 高频事件自动节流（低/中/高三档频率），不会刷屏；成功/失败/完成字幕带左侧状态色条
- 同类型事件从多条候选文案中随机选取，降低重复感
- 实时推送：安装包版使用 **SSE 常驻连接**（零延迟，自动重连），`/poll` 每 8s 兜底
- 设置页：启用开关、人物选择、宠物形象（种类/颜色/名字/装饰/大小）、字幕时长、
  出现频率、极轻提示音（默认关闭）、无障碍播报开关、预览字幕、恢复默认设置；
